...
def run():
    ...
    status, html, _ = request('/' + TAG.lower())
    check('ruta del QR abre la ficha del productor activo', status == 200 and 'window.TARGET_PRODUCER_PARAM = "' + TAG.lower() + '"' in html)
    status, html, _ = request('/admin/productor-form.php?from_solicitud=' + str(sid), data=producer)
    ...
    status, html, _ = request('/' + TAG.lower())
    check('ruta del QR no publica un productor inactivo', status == 404 and 'Productor no encontrado' in html)
